...
src/i18n.js                 the published feature names, en/fr, and the language
...
**The feature names are ours to translate, and they are written once.** Gladys labels a feature from its own dictionary ONLY when the feature type appears once in the device (`getDeviceFeatureName`, front/src/utils/device.js) — a unit carries three `energy-sensor/energy` features, two `temperature-sensor/decimal` and several `binary`, which all fall back to the published `name`; Settings → Energy renders that raw name and nothing else, whatever the type. The host API exposes no user language (`GET /status` = Gladys version + service status), so `src/i18n.js` holds every name in `{ en, fr }`, `device_language` picks one, and `auto` reads the `TZ` the supervisor injects (the Gladys system timezone) committing to French only for unambiguous zones — Belgium, Switzerland and Luxembourg stay English on purpose. Two consequences: a new feature key MUST get an entry in `FEATURE_NAMES` (`featureName()` throws otherwise, and a test compares both lists), and re-publishing NEVER renames a feature — `setDiscoveredDevices` only upserts params, and `structureDiffers` ignores names, so a language change reaches devices created afterwards or after the next structural change.
